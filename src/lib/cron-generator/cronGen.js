(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            //重写，编辑状态下，赋值
            var $i;
            if (options.scope.formData && options.scope.formData.cronExpression) {
                $i = $("<input>", {
                    type: 'text',
                    placeholder: 'cron表达式...',
                    readonly: 'readonly'
                }).addClass("form-control").val(options.scope.formData.cronExpression);
            } else {
                $i = $("<input>", {
                    type: 'text',
                    placeholder: 'cron表达式...',
    ...
    var generate = function (scope) {
        ...
        //重写 传入$scope,兼容angular
        scope.formData.cronExpression = results;
    };
